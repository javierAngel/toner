...
class toner
{
	...
	public function cantidadTonersEntregados(){
		$query="SELECT CONCAT(a.NOMBRE,' - ',a.LOCALIZACION) AS ALMACENS, CONCAT(t.NOMBRE,' ',t.DESCRIPCION) AS TONERS, ds.CANTIDAD, COUNT(ds.CANTIDAD) AS CANTIDAD
				...
				WHERE ds.ID_SURTIMIENTO=s.ID_SURTIMIENTO AND ds.ID_ALMACEN=a.ID_ALMACEN AND ds.ID_TONER=t.ID_TONER AND DATE_FORMAT(s.FECHA_SURTIMIENTO,'%c')=DATE_FORMAT(NOW(),'%c')
				GROUP BY ALMACENS";
